...
class redmag_twitter extends WP_Widget{

    /*function construct*/
    public function __construct() {
        parent::__construct(
            'twitter',esc_html__('+NA: Twitter','redmag'),
            array('description'=>esc_html__('Update news from your twitter', 'redmag'))
        );
    }
    /**
     * font-end widgets
    ...
    public function widget($args, $instance) {
        ...
        <div class="redmag-twitter">
                   <a class="twitter-timeline" href="https://twitter.com/<?php echo esc_attr($instance['username']);?>"
                       ...
                       <?php esc_html_e('Tweets by','redmag');?>
    ...
    public function form($instance){
        $instance =   wp_parse_args($instance,array(
            'title'             =>  esc_html__('Latest Tweets', 'redmag'),
            'username'          =>  'themenano',
            'width'             =>  '300',
            'height'            =>  '250',
            'theme'             =>  'dark',
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('username')); ?>"><?php echo esc_html_e('Twitter Username: :','redmag'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('theme')); ?>">
                <strong><?php esc_html_e('Theme: ', 'redmag') ?>:</strong>
                <select class="widefat" id="<?php echo esc_attr($this->get_field_id('type')); ?>"
                        name="<?php echo esc_attr($this->get_field_name('theme')); ?>">
                    <option
                        value="light"<?php echo (isset($instance['theme']) && $instance['theme'] == 'light') ? ' selected="selected"' : '' ?>><?php esc_html_e('Light', 'redmag') ?>
                    </option>
                    <option
                        value="dark"<?php echo (isset($instance['theme']) && $instance['theme'] == 'dark') ? ' selected="selected"' : '' ?>><?php esc_html_e('dark', 'redmag') ?>
                    </option>
                </select>
            </label>
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('width')); ?>"><?php echo esc_html_e('Width:','redmag'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('height')); ?>"><?php echo esc_html_e('Height:','redmag'); ?></label>
...
function redmag_twitter(){
    register_widget('redmag_twitter');
}
add_action('widgets_init','redmag_twitter');
?>