...
class redmag_author_post extends WP_Widget {
    public function __construct() {
        parent::__construct(
            'author_post',esc_html__('+NA:Post By Author ','redmag'),
            array('description'=>esc_html__('This is widget show the post made by Author', 'redmag'))
        );
    }
...
// Widget Backend
    public function form( $instance ) {
        $instance = wp_parse_args($instance,array(
            'title'         => esc_html__('By Author','redmag'),
            'number'        => '5',
            'type_post'     => 'views',
        ));
        // Widget admin form
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('type_post')); ?>">
                ...
                <select class="widefat" id="<?php echo esc_attr($this->get_field_id('type_post')); ?>"
                    ...
                    <option
                        value="news"<?php echo (isset($instance['type_post']) && $instance['type_post'] == 'news') ? ' selected="selected"' : '' ?>><?php esc_html_e('News', 'redmag') ?></option>
                    <option
...
function redmag_author_post(){
    register_widget('redmag_author_post');
}
add_action('widgets_init','redmag_author_post');